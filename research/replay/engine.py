...
class ReplayEngine:
    ...
    def replay_iterator(self, start_time: datetime, end_time: datetime) -> Iterator[dict[str, Any]]:
        """
        ...
        if self.source == "mock":
            ...
            filtered = [
                item for item in self.mock_data if start_time <= item.captured_at <= end_time
            ]
            # Sort by captured_at
            filtered.sort(key=lambda x: x.captured_at)

            for item in filtered:
                yield {"item_id": item.item_id, "city": item.city, "captured_at": item.captured_at}
        else:
