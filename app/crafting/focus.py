...
def optimize_focus(max_focus: int = 10000, spec_level: int = 0) -> dict:
    ...
    with get_db_session() as db:
        opps = (
            db.query(CraftingOpportunity, Item.item_value)
            .join(Item, CraftingOpportunity.item_id == Item.item_id)
            .filter(CraftingOpportunity.is_active == True, CraftingOpportunity.profit > 0)
            .all()
        )
