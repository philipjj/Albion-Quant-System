...
async def price_cmd(ctx, *, query: str):
    ...
    CITY_SHORT = {
        "Bridgewatch": "BRW", "Martlock": "MRT", "Lymhurst": "LYM",
        "Fort Sterling": "FST", "Thetford": "THF",
    }
        ...
        # Mode 2: name search — covers every item in the game
            ...
            if not matched:
                await ctx.send(
                    ...
                    f"Try a broader name or use a direct item ID."
                )
                return

            if len(matched) > 1:
                lines = [
                    f"`{i.item_id}` — {i.name} (T{i.tier}.{i.enchant})"
                    for i in matched[:10]
                ]
                if len(matched) == 11:
...
async def tier(ctx, tier_val: str = ""):
    ...
    if not tier_val:
        ...
        await ctx.send(f"ℹ️ Current tier lock: **{current}**\nUse `!tier <4-8>` to lock or `!tier all` to unlock.")
        return
    ...
    if tier_val.isdigit() and 1 <= int(tier_val) <= 8:
        ...
        await ctx.send(f"🔒 **Tier Lock Enabled**: Only showing opportunities for **Tier {state.tier_lock}** items.")
    else:
...
async def minbm(ctx, profit: int = None):
    ...
    from app.core import state
    if profit is None:
        await ctx.send(f"ℹ️ Current **Black Market Minimum Profit** is **{state.min_bm_profit:,}** silver.")
        return
    if profit < 0:
...
async def mincraft(ctx, profit: int = None):
    ...
    from app.core import state
    if profit is None:
        await ctx.send(f"ℹ️ Current **Crafting Minimum Profit** is **{state.min_craft_profit:,}** silver.")
        return
    if profit < 0:
        await ctx.send("❌ Profit cannot be negative.")
        return
...
async def scan(ctx, bm_flag: str = ""):
    ...
    await ctx.send(f"🚀 Starting manual scan for **{settings.active_server.value.upper()}**... (BM: {'enabled' if scan_bm else 'disabled'})")
    ...
    try:
        ...
        await ctx.send(f"✅ Scan complete: **{len(bm)}** BM, **{len(crafting)}** Crafting, **{len(arb)}** Arb opportunities found.")
    except Exception as e:
...
async def caravan_cmd(ctx, source: str, dest: str, weight: float = 1000.0):
    ...
    if not res["items"]:
        await ctx.send(f"⚠️ No profitable transport routes found from {source.capitalize()} to {dest.capitalize()}.")
        return
