...
from app.core.freshness import get_max_material_age_seconds
...
def detect_quality_inversion(
    ...
    """
    # Use tier-based age limit if not explicitly provided
    if max_age is None:
        max_age = get_max_material_age_seconds(item_id)
    inversions = []
    ...
    for i, q_low in enumerate(qualities):
        ...
        for q_high in qualities[i + 1:]:
            ...
            if high_sp <= 0 or high_age > max_age:
                continue
            ...
            if not allow_zero_volume and min_volume > 0:
                if high_vol < min_volume or low_vol < min_volume:
                    continue
            elif min_volume > 0:
                if (high_vol > 0 and high_vol < min_volume) or (low_vol > 0 and low_vol < min_volume):
                    continue

            # Outlier ratio check: if reference sell price is unrealistically wider than high_sp * max_price_ratio, skip pair
            if max_price_ratio > 0 and low_sp > 0 and low_sp > high_sp * max_price_ratio:
                continue
            ...
            if low_sp > 0 and is_price_valid(low_sp, low_bm, item_id=item_id) and high_sp < low_sp:
                # Anti-bait: ensure lower quality reference price is anchored or has confirmed volume
                if low_bm <= 0 and (low_vol <= 0 and not allow_zero_volume):
                    continue
                if low_bm > 0 and low_sp > (low_bm * 2.0):
                    continue
                if low_sp > high_sp * 1.5:
                    continue
                ...
                # Realistic intra-city quality inversion ceiling (max 35% ROI)
                if net_profit >= min_profit and profit_pct >= min_margin and profit_pct <= 35.0:
                    inversions.append({
                        "item_id": item_id,
            ...
            # Case 2: Higher quality sell price lower than lower quality buy order (Instant fill)
                ...
                if not (max_price_ratio > 0 and low_bm > high_sp * max_price_ratio):
