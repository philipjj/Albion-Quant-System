    ...
    async def get(
        ...
        while attempt <= max_retries:
            ...
            try:
                ...
                self.telemetry["status_codes"][code] = (
                    self.telemetry["status_codes"].get(code, 0) + 1
                )
                ...
                if resp.status_code in [429, 500, 502, 503, 504]:
                    ...
                    log.warning(
                        f"🌐 HTTP: Retry {attempt + 1}/{max_retries} for {url} in {wait:.2f}s (Status: {resp.status_code})"
                    )
