...
class TimescaleMarketDataRepository(IMarketDataRepository):
    """
    TimescaleDB implementation of IMarketDataRepository.
    ...
    async def save_snapshots(self, snapshots: List[MarketSnapshot]) -> None:
        ...
        for s in snapshots:
            ...
            stmt = insert(MarketPrice).values(
                ...
                city=s.city,
                quality=s.quality,
                server="west", # Default or get from settings
    ...
    async def get_latest_snapshot(self, item_id: str, city: str) -> Optional[MarketSnapshot]:
        stmt = select(MarketPrice).filter(
            MarketPrice.item_id == item_id,
            MarketPrice.city == city
        ).order_by(MarketPrice.captured_at.desc()).limit(1)
    ...
    async def update_volume(self, item_id: str, city: str, quality: int, volume: int) -> None:
        stmt = select(MarketPrice.id).filter(
            MarketPrice.item_id == item_id,
            MarketPrice.city == city,
        ...
        if row_id:
            ...
            stmt = update(MarketPrice).where(MarketPrice.id == row_id).values(volume_24h=volume)
            await self.session.execute(stmt)
