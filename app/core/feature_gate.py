...
class FeatureGate:
    ...
    def __new__(cls):
        if cls._instance is None:
            cls._instance = super(FeatureGate, cls).__new__(cls)
    ...
    def report_failure(self, endpoint: str, status_code: int):
        ...
        if status_code == 404:
            if endpoint == "orders" and self.orders_supported:
                ...
                log.warning(f"⚠️ [GATE] Orders endpoint returned 404. Disabling supply metrics for this session.")
            elif endpoint == "history" and self.history_supported:
                self.history_supported = False
                log.error(f"❌ [GATE] History endpoint returned 404. Demand verification will be estimated.")
                
        elif status_code == 429:
            self.is_rate_limited = True
            log.error(f"🚫 [GATE] RATE LIMITED (429). Throttling all requests.")
