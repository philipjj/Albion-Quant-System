...
class PatchParser:
    def parse_content(self, content: str) -> list[ParsedPatchChange]:
        ...
        buff_matches = re.findall(
            r"([A-Za-z\s]+?)\s*(?:damage|healing|cooldown reduction)?\s*(?:increased|buffed)",
            content,
            re.IGNORECASE,
        )
        ...
        nerf_matches = re.findall(
            r"([A-Za-z\s]+?)\s*(?:damage|healing|cooldown)?\s*(?:reduced|nerfed)",
            content,
            re.IGNORECASE,
        )
        for match in nerf_matches:
            ...
            if item:
                changes.append(
                    ParsedPatchChange(
                        item=item, change="nerf", severity=0.5, expected_market_impact="medium"
                    )
                )
