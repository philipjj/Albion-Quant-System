...
class PatchDiffEngine:
    ...
    def diff_spells(self, old_patch: dict[str, dict], new_patch: dict[str, dict]) -> pd.DataFrame:
        ...
        for spell_id, new_data in new_patch.items():
            ...
            d_dmg = self.compute_stat_diff(
                old_data.get("damage", 0), new_data.get("damage", 0), "damage"
            )
            d_cd = self.compute_stat_diff(
                old_data.get("cooldown", 0), new_data.get("cooldown", 0), "cooldown"
            )
            d_nrg = self.compute_stat_diff(
                old_data.get("energy_cost", 0), new_data.get("energy_cost", 0), "energy_cost"
            )
            d_coef = self.compute_stat_diff(
                old_data.get("coefficient", 0), new_data.get("coefficient", 0), "coefficient"
            )
